...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  const [strangerUnread, setStrangerUnread] = useState(false)
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(userRef, async (userSnap) => {
      const userData = userSnap.data() as { lastOpenedStrangerChatAt?: { toMillis?: () => number } } | undefined
      const openedAt = userData?.lastOpenedStrangerChatAt?.toMillis?.() ?? 0
      const conversationSnap = await getDocs(
        query(collection(db, 'conversations'), where('participantIds', 'array-contains', currentUser.uid), where('type', '==', 'stranger'))
      )
        ...
        <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-white/[0.06] bg-[#050816]/60 backdrop-blur-xl lg:flex">
          ...
          <nav className="flex-1 space-y-1 px-3">
            {navItems.map((item) => {
              ...
              const unreadDot = item.to === '/stranger-chat' && strangerUnread
              return (
